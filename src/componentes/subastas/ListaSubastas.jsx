...
function ListaSubastas({ soloActivas = false, limite }) {
  ...
  useEffect(() => {
    obtenerSubastas().then(r => setTodas(r.datos || [])).catch(() => {}).finally(() => setCargando(false));
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex flex-wrap gap-1">
          {[{k:'todas',l:'Todas'},{k:'activa',l:'Activas'},{k:'pendiente_pago',l:'Pendientes'},{k:'vendida',l:'Vendidas'},{k:'incumplida',l:'Incumplidas'}].map(e => (
            ...
            <Card key={s.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={()=>navigate(`/subasta/${s.id}`)}>
              <CardContent className="p-4 space-y-3">
                ...
                <div className="flex justify-between items-center">
                  <div><p className="text-xs text-muted-foreground">Precio actual</p><p className="text-lg font-bold text-primary">{s.precioActual?.toLocaleString('es-BO', { style: 'currency', currency: 'BOB' })}</p></div>
                  <Badge variant={ESTADO_VARIANTE[s.estado] || 'outline'}>{s.estado}</Badge>
                </div>
